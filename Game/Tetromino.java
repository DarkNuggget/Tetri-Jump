...
public class Tetromino {
    ...
    private int[][] shape;      // Form des Tetrominos
    private int x, y;           // Position des Tetrominos
    private Color color;        // Farbe des Tetrominos
            ...
            for (int col = 0; col < shape[row].length; col++) {
                if (shape[row][col] != 0) {
                    ...
                    if (newX < 0 || newX >= grid[0].length || newY >= grid.length) {
                        return false;  // Kollidiert mit Spielfeldgrenzen
                    }

                    if (newY >= 0 && grid[newY][newX] != null) {
                        return false;  // Kollidiert mit bereits existierenden Blöcken
                    }
                }
            }
        }
            ...
            for (int col = 0; col < shape[row].length; col++) {
                if (shape[row][col] != 0) {
                    ...
                    if (gridY >= 0 && gridY < grid.length && gridX >= 0 && gridX < grid[0].length) {
                        grid[gridY][gridX] = color;  // Die Farbe speichern
                    }
                }
            }
        }
